  ...
  if(isset($_GET['quiz']) && empty($_GET['quiz']) == false){
        ...
        if($gsubnum == 0){
          $ensubs = '<div class="center" style="margin-top: 50px;color: #999;">
                        <i class="sidebar-menu-icon material-icons" style="font-size: 50px;width: 70px;">info</i>
                        <p>Sorry you have not taken this quiz</p>
                        <a href="hub"><button type="button" class="btn btn-primary">
                          <i class="material-icons">home</i>
          ...
          $slq = mysqli_query($link, "SELECT slid,lesson_name,slcid FROM subject_level_curriculum WHERE slcid IN (SELECT slcid FROM lesson_structure WHERE mtype='q' AND mid='$qidz')");
            ...
            $qz = mysqli_query($link, "SELECT qqid,answer FROM student_queries WHERE qqid IN (SELECT qqid FROM queries WHERE qid='$qidz' ) AND stid='$uidx' ");
            ...
            if($qznum > 0){
              ...
              while($az = mysqli_fetch_array($qz, MYSQLI_ASSOC)){
                ...
                $qd = mysqli_query($link, "SELECT * FROM queries WHERE qqid='$qqidz' ");
                $ar = mysqli_fetch_assoc($qd);

                $qattach = '';
                $questionx = $ar['question'];
                if(empty($ar['attach'])==false){
                  if($ar['attach_type'] == 'i'){
                    $qattach = '<nav class="center">
                                  <img  src="assets/materials/'.$ar['attach'].'" style="width:50%;background-size:100%;">
                                </nav>';
                  }
                  else{
                    $qattach = '<nav class="center">
                                  <audio controls style="width: 100%;">
                                    <source src="assets/materials/'.$ar['attach'].'" type="audio/mpeg">
                                  </audio>
                                </nav>';
                  }
                }
                ...
                if($stanswer == $ar['answer']){
                  $queries .= '<p><b>'.$questionx.'</b>'.$qattach .'<br></p>
                              <div class="media">
                                <div class="media-body">'.$stanswer.'</div>
                                <div class="media-right"><span class="label label-success">Correct</span></div>
                              </div><hr>';
                  $countt++;
                }
                else{
                  $queries .= $queries .= '<p><b>'.$questionx.'</b>'.$qattach .'<br></p>
                              <div class="media">
                                <div class="media-body">'.$stanswer.'</div>
                                <div class="media-right"><span class="label label-danger">Wrong</span></div>
                              </div><hr>';;
                }
              }
              ...
              $lessonx = '<div class="card col-lg-12" style="margin-top:20px;padding-bottom: 15px;">
                            <p style="margin-bottom: 5px;margin-top: 20px;">Title: <span style="font-weight: bold;">'.$mr['title'].'</span></p>
                              <hr>
                              '.$queries.'
                              <div class="card card-footer" style="margin-bottom:10px">
                                Total Score: <span class="h5 text-primary"><strong>'.$countt.'/'.$qtnum.'</strong></span>
                              </div>
                            </div>
                            ';
